...
def draw_selection(image, roi, mode='rect', roi_color=(255,0,0), text_color=(255,255,0), select_point=None):
    ...
    if mode == 'rect':  # [x, y, w, h]
        ...
        if select_point and len(roi) >= 2:
            res = cv2.line(image, tuple(roi[0]), tuple(select_point), text_color, 2)
            res = cv2.line(res, tuple(roi[-1]), tuple(select_point), text_color, 2)
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if view_img:
                ...
                while roi_control.new_init or roi_control.status == 'select':
                    ...
                    vis = draw_selection(vis, roi, roi_selection, select_point=roi_control.cur_pt)
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    # valid in ROI
                    if roi and not roi[0] == [-1, -1]:
                        pt_x, pt_y = int((xyxy[0]+xyxy[2])/2), int(xyxy[3])
                        if roi_control.pt_in_roi(pt_x, pt_y):
                            if view_img:
                                cv2.circle(im0, (pt_x, pt_y), 10, roi_color, -1)
                        else:
                            continue
